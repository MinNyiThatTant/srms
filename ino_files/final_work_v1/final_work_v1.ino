...
void setup() {
  ...
  Serial.println("Ready. Press # then 1-4");
}
...
void followLineToTarget() {
  ...
  if (rawLeft == 0 && rawRight == 0 && !bothWhiteStarted) {
    bothWhiteStarted = true;
    Serial.println("Both white START");
  }
  
  // Detect both white ending (transition to black)
  if (bothWhiteStarted && rawLeft == 1 && rawRight == 1 && !markerPassed) {
    bothWhiteStarted = false;
    markerPassed = true;
    currentMarker++;
  ...
  // Reset marker flag when not both white
  ...
  if (rawLeft == 1 && rawRight == 1) {
    ...
    // Left black, right white - curve LEFT
    lastDirection = -1;
    inCurve = true;
    ...
    // Right black, left white - curve RIGHT
    lastDirection = 1;
    inCurve = true;
    ...
    // Both white - continue curve or search
    continueCurve();
  }
...
void followLineReturn() {
  ...
  // Marker detection (same transition logic)
  if (rawLeft == 0 && rawRight == 0 && !bothWhiteStarted) {
    bothWhiteStarted = true;
    Serial.println("Both white START (ret)");
  }
